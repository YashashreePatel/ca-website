...
const Hero: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'About' | 'Philosophy' | 'Mission & Vision' | 'Values' | 'Expertise' | 'Why Cogniify'>('About');
  return (
    <div className={`w-full flex flex-col gap-[80px] justify-center items-center px-[24px] py-[50px] tablet:px-[60px] desktop:px-[100px] desktop:py-[80px] bg-black`}>
      <div className="w-full flex flex-row flex-wrap gap-[24px] justify-center">
        {['About', 'Philosophy', 'Mission & Vision', 'Values', 'Expertise', 'Why Cogniify'].map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab as typeof activeTab)}
            className={`cursor-pointer bg-white/5 text-white text-center font-montserrat font-normal text-[14px] px-[40px] py-[12px] rounded-full border-2 transition-colors ${
              activeTab === tab
                ? 'border-[#634A92]'
                : 'border-white/15'
            }`}
